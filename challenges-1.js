...
// note: p = passenger when eslint is angry about the line being too long
...
const getTotalPassengers = (data) => {
  const totalPass = data.reduce((total) => {
    return total + 1;
  }, 0);
  return totalPass;
};
...
const getCasualtyCount = (data) => {
  const casualties = data.filter((passenger) => {
...
const countPassengersInClass = (data, pclass) => {
  const classList = data.filter((passenger) => {
...
const getSurvivorCountForClass = (data, pclass) => {
  const survivorClass = data.filter((passenger) => {
    return passenger.fields.pclass === pclass && passenger.fields.survived === 'Yes';
  });
  return survivorClass.length;
};
...
const getCasualtyCountForClass = (data, pclass) => {
  const casualtyClass = data.filter((passenger) => {
    return passenger.fields.pclass === pclass && passenger.fields.survived === 'No';
  });
  return casualtyClass.length;
};
...
const getMinAge = (data) => {
  const filteredAges = data.filter((p) => p.fields.age !== undefined).map((p) => p.fields.age);
  const minAge = Math.min(...filteredAges);
  return minAge;
};
...
const getMinFare = (data) => {
  const filteredFare = data.filter((p) => p.fields.fare !== undefined).map((p) => p.fields.fare);
  const minFare = Math.min(...filteredFare);
  return minFare;
};
...
const getMaxFare = (data) => {
  const filteredFare = data.filter((p) => p.fields.fare !== undefined).map((p) => p.fields.fare);
  const maxFare = Math.max(...filteredFare);
  return maxFare;
};
...
const getPassengersByGender = (data, gender) => {
  const genderList = data.filter((passenger) => {
...
const getTotalFare = (data) => {
  const totalFares = data.reduce((total, passenger) => {
...
const getAverageFare = (data) => {
  const filteredFares = data.filter((passenger) => passenger.fields.fare !== undefined);
  const totalFares = filteredFares.reduce((total, passenger) => {
    return total + passenger.fields.fare;
  }, 0);
  return totalFares / filteredFares.length;
};
...
const getMedianFare = (data) => {
  const filteredFares = data.filter((p) => p.fields.fare !== undefined).map((p) => p.fields.fare);
  const sortedFares = filteredFares.sort((a, b) => a - b);
  const medianFare = Math.floor(sortedFares.length / 2);

  if (sortedFares.length % 2 === 0) {
    const middle1 = sortedFares[medianFare];
    const middle2 = sortedFares[medianFare + 1];
    return (middle1 + middle2) / 2;
  }
  return sortedFares[medianFare];
};
...
const getAverageAge = (data) => {
  const filteredAges = data.filter((passenger) => passenger.fields.age !== undefined);
  const totalAge = filteredAges.reduce((total, passenger) => {
    return total + passenger.fields.age;
  }, 0);
...
const getMedianAge = (data) => {
  const filteredAges = data.filter((p) => p.fields.age !== undefined).map((p) => p.fields.age);
  const sortedAges = filteredAges.sort((a, b) => a - b);
  const medianAge = Math.floor(sortedAges.length / 2);

  if (sortedAges.length % 2 === 0) {
    const middle1 = sortedAges[medianAge];
    const middle2 = sortedAges[medianAge + 1];
    return (middle1 + middle2) / 2;
  }
  return sortedAges[medianAge];
};
...
const getAverageAgeByGender = (data, gender) => {
  const filteredP = data.filter((p) => p.fields.age !== undefined && p.fields.sex === gender);
  const totalAge = filteredP.reduce((total, p) => {
    return total + p.fields.age;
  }, 0);
  return totalAge / filteredP.length;
};
